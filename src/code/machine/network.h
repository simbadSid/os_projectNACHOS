...
class PacketHeader
{
  public:
    ...
    unsigned length;													 	// bytes of packet data, excluding the packet header (but including the
    																		//		MailHeader prepended by the post office)
};

#define MaxWireSize 	64													// largest packet that can go out on the wire
#define MaxPacketSize 	(MaxWireSize - sizeof(struct PacketHeader))			// data "payload" of the largest packet
...
class Network
{
  public:
    ...
    void SendDone();														// Interrupt handler, called when message is sent
  ...
  private:
    ...
    VoidFunctionPtr	writeHandler;											// Interrupt handler, signalling next packet can be sent.
    VoidFunctionPtr readHandler;											// Interrupt handler, signalling packet has arrived.
    int				handlerArg;												// Argument to be passed to interrupt handler (pointer to post office)
    bool			sendBusy;												// Packet is being sent.
    bool			packetAvail;											// Packet has arrived, can be pulled off of network
